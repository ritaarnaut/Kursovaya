...
def download_report():
    ...
    report_window.title("Скачать отчет")
    report_window.geometry("300x200")

    tk.Label(report_window, text="Выберите дату", font=("Arial", 12)).pack(pady=10)

    date_entry = ttk.Entry(report_window)
    date_entry.pack(pady=5)
    date_entry.insert(0, datetime.now().strftime("%Y-%m-%d"))  # Предлагаем текущую дату по умолчанию
        ...
        if file_path:
            try:
                with sqlite3.connect("restaurant.db") as conn:
                    cursor = conn.cursor()
                    cursor.execute("""
                        SELECT id, order_details, total_price
                        FROM orders
                        WHERE date(order_date) = ?
                    """, (selected_date,))
                    orders = cursor.fetchall()
                    with open(file_path, 'w') as report_file:
                        report_file.write(f"Отчет о заказах за {selected_date}:\n")
                        if orders:
                            for order in orders:
                                report_file.write(f"Заказ {order[0]} | {order[1]} - {order[2]} руб.\n")
                        else:
                            report_file.write("Заказов за выбранную дату нет.\n")
                    messagebox.showinfo("Успех", f"Отчет сохранен: {file_path}")
            except sqlite3.Error as e:
                messagebox.showerror("Ошибка", f"Ошибка при создании отчета: {e}")
        report_window.destroy()

    tk.Button(report_window, text="Скачать", command=save_report).pack(pady=10)
    tk.Button(report_window, text="Отмена", command=report_window.destroy).pack(pady=5)
